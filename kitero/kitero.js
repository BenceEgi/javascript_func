...
function hideBasedOnSelected(dropdownList){
    const parentDiv = dropdownList.parentElement;
    const paramValue = dropdownList.value;
    const cards = parentDiv.querySelectorAll(".card");

    for (const card of cards){
        const id = card.getAttribute("id");
        makeInvisibleBasedOnid(parentDiv, id);
    }
    makeVisibleBasedOnId(parentDiv, paramValue);
}
...
function hideBasedOnCheckbox(checkbox){
    ...
    const paramValue = checkbox.checked;
    const cards = parentDiv.querySelectorAll(".card");
    ...
    makeVisibleBasedOnId(parentDiv, paramValue ? "ferfi" : "no");
}
